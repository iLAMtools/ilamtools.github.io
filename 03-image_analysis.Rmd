...
```
  ...
  - Update values for every cage and/or experiment:
    ...
    - Experiment-Specific:
      - n_thr: Numeric threshold percentile; higher values indicate a more stringent filtering. e.g., at n_thr=0.999, only the darkest pixel differences > 99.9% will be retained for segmentation 
      - n_cln: Integer value to clean up pixels; e.g., at n_cln = 5, this first "shrinks" to remove all isolated blobs smaller than 5 pixels large, and then grows remaining globs by 5 pixels to aggregate nearby blobs with eachother. See [imager documentation](https://asgr.github.io/imager/morphology.html) for more details. 
      - genus: Character string for study organis,m
