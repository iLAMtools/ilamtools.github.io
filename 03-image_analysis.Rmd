...
```
  ...
  - Update values for every cage and/or experiment; these will be transferred to metadata output and fed as input settings to the find_movements() iLAM wrapper function:
...
```
  ...
  - Find all movements by image subtraction, global thresholding, and blob detection in the iLAM wrapper function ```find_movements()```. 
    -Additional *optional* settings:
      ...
      - *n_max:* Integer value denoting the maximum number of pixel differences expected between two images; any values above this n_max will be considered as noisy and assigned an arbitrary value for subsequent filtering/removal. See \@ref(04-make_dam_output) for more information.   Example, if a moth movement = 3000 pixels and there are five moths in a cage, then the maximum number of differences expected is 15,000 pixels. (default: n_max = 75000)
