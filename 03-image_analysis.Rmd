...
```
  ...
  - Update values for every cage and/or experiment:
    - Cage-Specific: Imaging cropping coordinates (x_left, x_right, y_bot, y_top)
    - Experiment-Specific:
      - n_thr: Threshold percentile; higher values indicate a more stringent filtering. e.g., at n_thr=0.999, only the darkest pixel differences > 99.9% will be retained for segmentation 
      - n_cln: Value to clean up pixels; e.g., at n_cln = 5, this first "shrinks" to remove all isolated blobs smaller than 5 pixels large, and then grows remaining globs by 5 pixels to aggregate nearby blobs with eachother. See [imager documentation](https://asgr.github.io/imager/morphology.html) for more details. 
      - Species Characters: genus, species, color
      - 
```
